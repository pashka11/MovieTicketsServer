...
//        if we got here then show instance is valid ,so we extract the theater document from it
        Document theaterDocument = theaterCrud.read( new ShowInstanceDao().read(filterBean.getShowInstanceID()).get(SHOW_INSTANCE_THEATER_ID).toString());
